...
const qutil = require('../core/qutil');
const router = require('./router');
const UserDirsRouter = require('./user-dirs-router');
const config = require('../config/config');
const posix = require('../../build/Release/posix');
const DirectoryRouter = require('./directory-router');
const throwForNode = require('../config/schema').throwForNode;
const configRouterUtil = require('./config-router-util');
...
(function() {
  ...
  this.getScore = function(req, res) {
    ...
    if (iputil.isWildcard(this.host))
      score += 1;
    else if (iputil.equal(this.host, host))
      score += 2;
    else
      return 0;
